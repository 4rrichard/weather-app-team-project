...
function Weather() {
    ...
    useEffect(()=> {
        const cityListFetch = async ()=> {
            const cityListResponse = await fetch(`http://api.weatherapi.com/v1/search.json?key=9ccc715638434575be273925221605&q=${citySearch}`)
    ...
    useEffect(() => {
        const weatherFetch = async ()=> {
            ...
            const response = await fetch(`http://api.weatherapi.com/v1/current.json?key=9ccc715638434575be273925221605&q=${cityName}`);
    ...
    useEffect(()=> {
        const imageFetch = async ()=> {
            const response = await fetch(`https://api.pexels.com/v1/search?query=${cityName}`, {
                headers: {
                    Authorization: "563492ad6f917000010000016a38353128cd4cff95e5db1ee80e0e57"
                }
            })
            const data = await response.json();
    ...
    return (
        <div className="widget-section">
            ...
            <div className="search">
                <input type="text" placeholder="city" list="chosen-cities" onChange={(event)=>{setCityName(event.target.value); setCitySearch(event.target.value); setSubmit(""); setFavouriteClassName('favourite-off');setBackgroundImage(""); setCityList([])}}/>
                <button id="submit" onClick={()=>{setSpinner(true); setTimeout(()=>{setSubmit("reset"); setSpinner(false)}, 2000)}}>Submit</button>
                <button  className={favouriteClassName} onClick={()=>{
